...
//Accounts for cases:
  ...
  //2. The user has just made a trip using createTrip and user_id and trip_id are stored
  //   on res.locals 
...
userController.updateUserTrips = async (req, res, next) => {
  ...
  const { trip_id } = res.body || res.locals || {trip_id : undefined};

  // const { user_id } = res.body // grab the trip
  const { date } = res.locals.trip.date || res.body.date //***** case 2. not on createTrip */
  const { tripName } = res.locals.trip || res.body.tripName // grabs the name of the trip
